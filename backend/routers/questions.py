...
def get_questions(
    difficulty: Optional[str] = Query(None, description="Difficulty level: basic, intermediate, advanced"),
    language: Optional[str] = Query(None, description="Programming language: Python, C++, JavaScript"),
...
def get_user_progress(
    ...
    progress = session.exec(select(UserQuestionProgress).where(UserQuestionProgress.user_id == user.id)).all()
    return {p.question_id: {"attempted": p.attempted, "solved": p.solved, "last_answer": p.last_answer} for p in progress}
...
def update_user_progress(
    ...
    progress = session.exec(select(UserQuestionProgress).where(
        (UserQuestionProgress.user_id == user.id) & (UserQuestionProgress.question_id == question_id)
    )).first()
    if not progress:
...
def submit_answer(question_id: int, answer: str = Body(..., embed=True), session: Session = Depends(get_session)):
    question = session.get(Question, question_id)
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")
